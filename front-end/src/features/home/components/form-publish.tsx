...
export const FormPublish = () => {
  ...
  useEffect(() => {
    if (competitionData?.data?.isPublic == 1) {
      setIsPublic(true);
    }
  }, [competitionData?.data?.isPublic]);
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(`${import.meta.env.VITE_DOMAIN_APP}/competition/cuoc-thi/intro/${id}/${slug}`);
  ...
  const showConfirm = () => {
    if (isPublic) {
      Modal.confirm({
        title: 'Xác nhận',
        content: 'Bạn có chắc chắn muốn thu hồi cuộc thi này không?',
        onOk: handleSubmit,
        okText: 'Xác nhận',
        onCancel() {},
        cancelText: 'Huỷ'
  ...
  return (
    <div className='-mt-10'>
      <div className='mb-4'>
        <label className='block mb-1 text-[15px] text-[#757575] font-medium'>Link rút gọn</label>
        <Input
          size='large'
      ...
      <div>
        <label className='block mb-1 text-[15px] text-[#757575] font-medium'>Mã QR bài thi</label>
        <QRCode
          value={`${import.meta.env.VITE_DOMAIN_APP}/competition/cuoc-thi/intro/${id}/${slug}` || '-'}
          size={250}
        />
