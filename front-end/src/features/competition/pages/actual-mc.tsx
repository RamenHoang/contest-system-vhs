...
const Quiz = () => {
  ...
  const [remainingTime, setRemainingTime] = useState(
    testDuration ? testDuration * 60 : null,
  ); // Convert to seconds
  ...
  const handleSubmit = (autoSubmit = false) => {
    ...
    const detailedResultData = questions.map(
      (
        question: { id: number; type: string; answers: { id: number }[] },
        index: number,
      ) => ({
        questionId: question?.id,
        chosenAnswerId:
          question?.type === "MC"
            ? question?.answers[selectedOptions[index]]?.id
            : null,
    ...
    <div>
      <div className="bg-cyan-700 sticky min-h-[80px] text-base top-0 z-10 flex items-center">
        <div className="grow flex justify-between items-center py-5 px-3 h-full">
          ...
          <div className="bg-white text-theme-color py-1 px-3 text-center min-w-[100px] rounded-2xl">
            {testDuration && remainingTime !== null
      ...
      {!isFetching ? (
        <div
          ...
          className="bg-white rounded-t-3xl md:rounded-none"
        >
          <div className="grid grid-cols-12 md:gap-4 p-4 text-sm lg:text-base">
            <div className="col-span-12 md:col-span-4 xl:col-span-3 sticky top-[80px] px-0 z-[1] md:order-2">
              <Button
                className="uppercase text-base font-light w-full py-5"
                onClick={showConfirmSubmitModal}
              >
                Nộp bài
              </Button>
                ...
                {questions.map((_: unknown, index: number) => (
                  <div key={index} className="flex flex-col items-center">
                    <Button
                      ...
                      style={{
                        backgroundColor:
                          selectedOptions[index] !== null || essayAnswers[index]
                            ? "#0e76aa"
                            : "lightgray",
                        borderColor:
                          selectedOptions[index] !== null || essayAnswers[index]
                    ...
                    {index === currentQuestion && (
                      <div className="solid #0e76aa">_</div>
                    )}
                  </div>
                ))}
              </div>
